...
import UserResponseContext from "../context/UserResponseContext";
const UserEnglishLesson = () => {
  ...
  const {  UserEassyFetch } = useContext(UserResponseContext);

  // Eassy handel submit-------
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const handelUserEassy = async (e) => {
    e.preventDefault();
    setLoading(true);
    await UserEassyFetch();
    setLoading(false);
        ...
        <div className="container mx-auto mt-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="lesson-form">
              <form onSubmit={handelUserEassy}>
                <div className="title text-center mt-3 mb-3">
                  <h1>
                    <MdOutlineMessage className="inline text-rose-500 text-3xl me-2" />
                    Letter|Eassy
                  </h1>
                </div>
                <div className="mx-8 mb-3">
                  <div className="mb-2 block">
                    <Label htmlFor="esy_type" value="Type" />
                  </div>
                  <Select id="esy_type" required>
                    <option>Choose Type...</option>
                    <option className="Eassy">Eassy</option>
                    <option className="Letter">Letter</option>
                  </Select>
                </div>
                ...
                <div className="col mt-8 mb-5 mx-8">
                  <Button type="submit" color="blue" className="w-full">
                    <i className="fa-solid fa-paper-plane me-2"></i>Create it
                  </Button>
                </div>
              </form>
            </div>
